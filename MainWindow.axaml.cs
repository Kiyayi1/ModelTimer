...
public partial class MainWindow : Window
{
    ...
    private void UpdateDisplay()
    {
        ...
        if (!_isRunning && _elapsed == TimeSpan.Zero)
        {
            RemainingText.Text = "00:00:00";
        }
        else
        {
            RemainingText.Text = remaining.ToString(@"hh\:mm\:ss");
        }
    ...
    private void StartResume_Click(object? sender, RoutedEventArgs e)
    {
        if (!_isRunning && !_isPaused)
        {
            ...
            _newShiftWindow.Closed += (s, args) =>
            {
                ...
                {
                    var totalMinutes = _newShiftWindow.DurationHours * 60 + _newShiftWindow.DurationMinutes;
                    _totalTime = TimeSpan.FromMinutes(totalMinutes);
                    _elapsed = TimeSpan.Zero;
                    _isRunning = true;
                    _isPaused = false;
                    _currentModel = _newShiftWindow.SelectedModel;
                    _currentModerator = _newShiftWindow.SelectedModerator;
